...
const API_BASE_URL = process.env.REACT_APP_API_BASE_URL || 'http://localhost:30000';
...
const useChat = () => {
  ...
  const sendMessage = useCallback(async (inputMessage, settings) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/easyllmops_conversation/conversation`, {
        method: 'POST',
      ...
      if (settings.isStreaming) {
        ...
        while (true) {
          ...
          const nextResponse = accumulatedResponse;

          setMessages(prevMessages => {
            const updatedMessages = [...prevMessages];
            updatedMessages[updatedMessages.length - 1] = { role: 'assistant', content: nextResponse, timestamp: new Date() };
